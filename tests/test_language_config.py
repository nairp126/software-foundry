...
REQUIRED_KEYS = {"name", "extension", "test_framework", "web_framework", "package_manager"}
...
class TestLanguageConfigs:
    ...
    def test_supported_language_returns_config(self, language):
        """Each supported language returns a non-None config dict."""
        config = get_language_config(language)
        assert isinstance(config, dict)

    @pytest.mark.parametrize("language", SUPPORTED_LANGUAGES)
    def test_config_has_all_required_keys(self, language):
        """Each config contains all five required keys."""
        config = get_language_config(language)
        for key in REQUIRED_KEYS:
            assert key in config, f"Key '{key}' missing from {language} config"
    ...
    def test_python_config_values(self):
        ...
        assert config["web_framework"] == "fastapi"
    ...
    def test_javascript_config_values(self):
        ...
        assert config["web_framework"] == "express"
    ...
    def test_java_config_values(self):
        ...
        assert config["extension"] == ".java"
        assert config["test_framework"] == "junit"
        assert config["web_framework"] == "spring"
